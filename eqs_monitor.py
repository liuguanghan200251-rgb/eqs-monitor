...
def check_update(root):
    def run():
        try:
            ...
            latest = resp.text.strip().replace("\r","").replace("\n","")
            current = VERSION.strip()
            if not latest:
                return
            # 比较版本号（转为数字比较，避免字符串问题）
            def ver_tuple(v):
                try:
            ...
            if ver_tuple(latest) > ver_tuple(current):
                ...
                if answer:
                    ...
                    if is_exe:
                        # exe 模式：把新代码写到临时 py 文件，用 Python 运行
                        current_exe = os.path.abspath(sys.argv[0])
                        temp_py = os.path.join(os.path.dirname(current_exe), "_update_temp.py")
                        with open(temp_py, "w", encoding="utf-8") as f:
                            f.write(new_code)
                        messagebox.showinfo("更新完成",
                            "更新成功！程序将自动重启。", parent=root)
                        # 找系统 Python
                        python = "python"
                        subprocess.Popen([python, temp_py],
